/* Initialisation code originally adapted from Traffic++ Initialisation:
 * 
...
namespace PitLane
{
	...
	class Initializer : MonoBehaviour
	{
		...
		{
			if (level == 6) {
				Debug.Log ("PitLane: Game level was loaded");
 ...
 */
		...
		static T ClonePrefab<T> (string prefabName, T[] prefabs, string newName, Transform customPrefabsHolder, bool replace = false, bool ghostMode = false) where T : PrefabInfo
		{
			...
			GameObject instance = UnityEngine.Object.Instantiate<GameObject> (originalPrefab.gameObject);
			...
			if (ghostMode) {
				Initializer.QueuePrioritizedLoadingAction ((IEnumerator)initMethod.Invoke (null, new object[] {
					newName,
					new[] { instance.GetComponent<T> () },
					new string[] { replace ? prefabName : null }
				}));
				return null;
			}
		...
		{
			NetInfo road = ClonePrefab<NetInfo> (prefabName + GetDecoratedName (roadType & ~RoadType.OneWay), collection.m_prefabs, newName, transform, false);
			if (road == null)
				return null;
		...
		#region Small Roads
		// TODO: Use this to create a custom race track type
		/*
...
*/
	...
	static class PrefabCloner
	{
		public static T ShallowCopy<T>(T source) where T : UnityEngine.Object
		{
			T ret = UnityEngine.Object.Instantiate<T> (source);

			foreach (FieldInfo fi in typeof(T).GetFields()) {
				if (!Equals(fi.GetValue (source),fi.GetValue (ret))) {
					fi.SetValue (ret, fi.GetValue (source));
				}
			}
			return ret;
		}

		public static BuildingInfo CloneBuilding(BuildingInfo source)
		{
			return ShallowCopy (source);
		}

		public static TransportInfo CloneTransport(TransportInfo source)
		{
			return ShallowCopy (source);
		}
	}
