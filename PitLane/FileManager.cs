...
namespace PitLane
{
	static class FileManager
	{
		...
		{
			PluginManager.PluginInfo plugin = Singleton<PluginManager>.instance.GetPluginsInfo().FirstOrDefault(p => p.name == "PitLane");
			if (plugin != null)
				return plugin.modPath;
			else
				Debug.Log("PitLane: Cannot find plugin path.");
		...
		public static bool GetTextureBytes(string fileName, Folder folder, bool skipCache, out byte[] bytes)
		{
			...
			{
				#if DEBUG
				Debug.Log("PitLane: Cannot find texture file at " + filePath);
				#endif
				return false;
			}
			...
			{
				Debug.Log("PitLane: Unexpected " + e.GetType().Name + " reading texture file at " + filePath);
				return false;
			}
